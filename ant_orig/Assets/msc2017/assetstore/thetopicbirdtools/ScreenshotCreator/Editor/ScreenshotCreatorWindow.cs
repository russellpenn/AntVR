...
public class ScreenshotCreatorWindow : EditorWindow {
	...
	void fileTypeGUI(){
		...
		GUI.color = fileType == ScreenshotCreatorWindow.FileType.JPG ? Color.white : (Color.grey + Color.white) / 2f;
		if (GUILayout.Button ("PNG", "toolbarButton")) {
			refreshRequests();
			fileType = ScreenshotCreatorWindow.FileType.PNG;
		}

		GUI.color = fileType == ScreenshotCreatorWindow.FileType.JPG ? (Color.grey + Color.white) / 2f : Color.white;
		if (GUILayout.Button ("JPG", "toolbarButton")) {
			refreshRequests();
			fileType = ScreenshotCreatorWindow.FileType.JPG;
		}
