...
public class suncal : MonoBehaviour {
	...
	void Update () {
		...
		SPACalculator.SPACalculator.SPAData spa = new SPACalculator.SPACalculator.SPAData ();
		//intial time
		DateTime starttime = new DateTime(initialtime.year,initialtime.month,initialtime.day,initialtime.hour,initialtime.minute,(int)initialtime.second);
		//current time
		DateTime nowtime=starttime.AddSeconds(Time.time*speed);
	    
		//Input for solar position algorithm
		spa.Year = nowtime.Year;
		spa.Month = nowtime.Month;
		spa.Day = nowtime.Day;
		spa.Hour = nowtime.Hour;
		spa.Minute = nowtime.Minute;
		spa.Second=nowtime.Second;
		spa.Timezone = timezone;

		spa.DeltaUt1 = 0;
		spa.DeltaT = 0;

		spa.Longitude = location.longitude;
		spa.Latitude = location.latitude;
		...
		spa.Function = SPACalculator.SPACalculator.CalculationMode.SPA_ZA;
		var result = SPACalculator.SPACalculator.SPACalculate(ref spa);

		//check for SPA errors
		if (result == 0)
		{
			float altitude = 90 - (float)spa.Zenith;
			float azimuth = (float)spa.Azimuth;
			//set the rotation of the directional light based on the real sun position calculated
			transform.rotation = Quaternion.Euler (altitude, azimuth, 0);
		}

		//output the current time
		currenttime.year = nowtime.Year;
		currenttime.month = nowtime.Month;
		currenttime.day = nowtime.Day;
		currenttime.hour = nowtime.Hour;
		currenttime.minute = nowtime.Minute;
		currenttime.second = nowtime.Second;

		//change the sunlight intensity based on the angle of incidence
		//float dot = Mathf.Clamp01 (Vector3.Dot (transform.forward, Vector3.down));
		//sun.intensity = minintensity + (maxintensity - minintensity) * dot;
...
//The following code is from http://github.com/xeqlol/SolarPositionAlgorithm
